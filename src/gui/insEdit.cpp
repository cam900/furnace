...
void FurnaceGUI::drawMacroEdit(FurnaceGUIMacroDesc& i, int totalFit, float availableWidth, int index) {
  ...
  if ((i.macro->open&6)==0) {
    ...
    if (MACRO_VZOOM<1) {
      if ((i.macro->macroType==DIV_MACRO_ARP || i.macro->macroType==DIV_MACRO_WM_ARP) || i.isArp) {
        MACRO_VZOOM=24;
        MACRO_VSCROLL=120-12;
      }
      else if (((i.macro->macroType == DIV_MACRO_PITCH || i.macro->macroType == DIV_MACRO_WM_PITCH) || i.isPitch) || (i.macro->macroType == DIV_MACRO_EX7 && i.isPitch)) {
        MACRO_VZOOM=128;
...
void FurnaceGUI::insTabWM(DivInstrument* ins) {
  if (ImGui::BeginTabItem("JKMS16WM32O8")) {
    ...
    if (ImGui::BeginTable("WMOperators",columns,ImGuiTableFlags_SizingStretchSame)) {
      for (int i=0; i<8; i++) {
        ...
        if (isFixed) {
          ImGui::SameLine();
          pushWarningColor(isPitchControl && !e->song.linearPitch);
          if (ImGui::Checkbox(_("Pitch control"),&isPitchControl)) { PARAMETER
            op.pitchCtrl=isPitchControl;
            // HACK: reset zoom and scroll in fixed pitch macros so that they draw correctly
            ins->temp.vZoom[DIV_MACRO_WM_PITCH+(i<<7)]=-1;
          }
          popWarningColor();
          if (ImGui::IsItemHovered()) {
            if (isPitchControl && !e->song.linearPitch) {
              ImGui::SetTooltip(_("only works on linear pitch! go to Compatibility Flags > Pitch/Playback and set Pitch linearity to Full."));
            } else {
